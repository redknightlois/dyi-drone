...
Enclosure features:
- Hollow box with open top (74 x 42 x 23 mm, 2mm walls)
...
BOSS_CLEARANCE = 2.0    # mm - gap between boss and inner wall for PCB insertion

BOX_L = 64.0            # mm (X) - fits PCB at scale
BOX_W = 42.0            # mm (Y) - must exceed 35mm PCB + clearance
...
def create_enclosure():
    ...
    Open-top box (74 × 42 × 23 mm, 2mm walls) with:
    ...
    Coordinate system (origin at center-bottom of box):
        X: ±BOX_L/2 = ±37     (short end walls at ±X)
        Y: ±BOX_W/2 = ±21     (long side walls at ±Y)
...
def create_lid():
    ...
    inner_l = BOX_L - 2 * WALL   # 70mm
    inner_w = BOX_W - 2 * WALL   # 38mm
    ...
    led_in_x = -DISPLAY_LENGTH / 4             # -7.5mm (left of center)
    led_out_x = DISPLAY_LENGTH / 4             # +7.5mm (right of center)

    # Cable holes: at +X wall, aligned with terminal cutout
    cable_x = inner_l / 2 - CABLE_HOLE_DIA / 2 - 3  # 3mm inset from inner wall
    cable_y1 = CABLE_HOLE_SPACING / 2              # two holes, 5mm apart
    cable_y2 = -CABLE_HOLE_SPACING / 2

    # Pot: below display, left of center (+5mm X, -5mm Y from previous)
    pot_x = -inner_l / 6 + STANDOFF_DIA - 2     # ~-8.7mm, shifted -X
    pot_y = -inner_w / 4 - STANDOFF_DIA        # ~-14.5mm

    # Reset button: at X=-25, +Y edge aligned with display top edge
    ...
    with BuildPart() as lid:
        ...
        # =================================================================
        # Step 5: Cable holes — far left edge
        # =================================================================
        with BuildSketch(Plane.XY.offset(LID_THICKNESS)):
        ...
        # =================================================================
        # Step 7: Reset button hole (square) — at X=-25, +Y aligned with display
        # =================================================================
